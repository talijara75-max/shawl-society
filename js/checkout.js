  ...
  list.innerHTML = cart.map(item => {
    ...
    return `
      <li class="preview-item">
        <span class="preview-item__name">${item.name}${variantLabel} × ${item.qty}</span>
...
function handlePlaceOrder() {
  ...
  if (!city || city === "")
                { showToast("Please select your city.", "error"); return; }
...
function buildWhatsAppMessage(order) {
  ...
  order.items.forEach(item => {
    const variantLabel = item.variant ? ` (${item.variant})` : "";
    lines.push(
      `• ${item.name}${variantLabel} × ${item.qty}  —  ${CONFIG.CURRENCY} ${item.price.toLocaleString()} each  =  ${CONFIG.CURRENCY} ${(item.price * item.qty).toLocaleString()}`
    );
  });
  lines.push("");
